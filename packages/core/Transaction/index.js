...
import { cfxSignTransaction, ethSignTransaction, getTxHashFromRawTx } from '@fluent-wallet/signature';
import { detectCfxAddressType, detectEthAddressType } from '@fluent-wallet/detect-address-type';
...
import { toEthersTx, FEE_HISTORY_BLOCKS, FEE_HISTORY_PERCENTILES, getGasFeeByGasStation, calculateGasFeeEstimatesForPriorityLevels } from './utils';
...
class Transaction {
  ...
  async estimateEth1559Fee() {
    ...
    try {
      ...
      const feeData = await this.getEthFeeHistory([FEE_HISTORY_BLOCKS, 'latest', FEE_HISTORY_PERCENTILES]);
  ...
  async sendRawTransaction(raw) {
    return this.send(this.isCfx ? 'cfx_sendRawTransaction' : 'eth_sendRawTransaction', [raw]);
  }
  ...
  async signCfxTransaction(tx, addressRecord) {
    ...
    if (!newTx.gas || !newTx.storageLimit) {
      try {
        const { gasLimit, storageCollateralized } = await this.getCfxEstimateGasAndCollateral([newTx]);
  ...
  async signEthTransaction(tx, addressRecord, block) {
    ...
    if (is1559Tx && !network1559Compatible) {
      throw Error(`Network ${this.network.name} don't support 1559 transaction`);
    }
    ...
    if (is1559Tx && network1559Compatible) {
      ...
      const { suggestedMaxPriorityFeePerGas, suggestedMaxFeePerGas } = gasInfoEip1559?.medium || {};
      if (!newTx.maxPriorityFeePerGas) {
        newTx.maxPriorityFeePerGas = parseUnits(suggestedMaxPriorityFeePerGas, 'gwei').toHexString();
      }
      if (!newTx.maxFeePerGas) {
        newTx.maxFeePerGas = parseUnits(suggestedMaxFeePerGas, 'gwei').toHexString();
      }
    }
    let raw;
  ...
  async sendTransaction({ tx, sendAction, token }) {
    ...
    const addressRecord = await getAddressByValueAndNetworkId(tx.from, this.network.id);

    if (!addressRecord) {
      throw Error(`Invalid from address ${tx.from}`);
    }
    const signTxFn = this.isCfx ? this.signCfxTransaction.bind(this) : this.signEthTransaction.bind(this);
    let signed;
    try {
