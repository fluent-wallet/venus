...
export function calculateGasFeeEstimatesForPriorityLevels(feeData, baseFeePerGas) {
  const levelSpecificEstimates = PRIORITY_LEVELS.reduce((obj, priorityLevel) => {
    const gasEstimatesForPriorityLevel = calculateEstimatesForPriorityLevel(priorityLevel, feeData, baseFeePerGas);
...
export function calculateEstimatesForPriorityLevel(priorityLevel, feeData, baseFeePerGas) {
  const settings = SETTINGS_BY_PRIORITY_LEVEL[priorityLevel];
  const adjustedBaseFeePerGas = baseFeePerGas.mul(settings.baseFeePercentageMultiplier).divn(100);
  const priorityFees = feeData.reward?.map((rewards) => new BN(Number(rewards[PRIORITY_LEVELS.indexOf(priorityLevel)]))).filter(BN.isBN);
  const adjustedPriorityFee = medianOf(priorityFees);
  const suggestedMaxPriorityFeePerGas = BN.max(adjustedPriorityFee, settings.minSuggestedMaxPriorityFeePerGas);
  const suggestedMaxFeePerGas = adjustedBaseFeePerGas.add(suggestedMaxPriorityFeePerGas);
  return {
    suggestedMaxPriorityFeePerGas: formatUnits(suggestedMaxPriorityFeePerGas.toString(), 'gwei'),
    suggestedMaxFeePerGas: formatUnits(suggestedMaxFeePerGas.toString(), 'gwei'),
  };
}
...
export async function getGasFeeByGasStation(chainId) {
  ...
  if (!res.ok) {
    throw new Error(`Fetch failed with status '${res.status}' for request gasFeeApi`);
  }
  return res.json();
}
