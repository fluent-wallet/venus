...
class Token20 {
  ...
  async initTokenToCurrentNetwork() {
    ...
    const isTokenInit = await database.localStorage.get(`is_${currentNetwork[0].chainId}_${currentNetwork[0].networkType}_token_init`);
    ...
    const tokenTableInstance = token.tokens.map(({ name, symbol, decimals, logoURI, address }) =>
    ...
    await database.localStorage.set(`is_${currentNetwork[0].chainId}_${currentNetwork[0].networkType}_token_init`, 'yes');
  }
}
