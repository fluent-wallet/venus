...
export const getAddressByValueAndNetworkId = async (value, network_id) => {
  const ret = await database
    .get('address')
    .query(Q.and(Q.where('value', value), Q.where('network_id', Q.eq(network_id))))
    .fetch();
  return ret[0];
};

export const getTxByAddrAndHash = async (hash, address_id) => {
  return database.get('tx').query(Q.and(Q.where('hash', hash), Q.where('address_id', Q.eq(address_id))));
};
