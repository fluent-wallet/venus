...
import { generateAddressesByMnemonic, preCreateAccount, preCreateAddress } from '../utils';
import { getAccountGroupVault, getNetworks, fetchAllRecord } from '../Query';

class Account {
  updateAccountGroupName({ nickname, accountGroupId }) {
    return database.write(async () => {
      const accountGroupRecord = await database.get(TableName.AccountGroup).find(accountGroupId);
  ...
  async addAccount({ accountGroupId, password }) {
    const accountGroupRecord = await database.get('account_group').find(accountGroupId);
  ...
  async eraseAllAccounts() {
    const allRecords = await Promise.all(Object.keys(DB_TABLE_SCHEMAS_OBJ).map((tableName) => fetchAllRecord(tableName)));
    const deleteRecordsArr = allRecords.flat();
    if (!deleteRecordsArr.length) {
      return;
    }
    await database.write(async () => {
      await database.batch(...deleteRecordsArr.map((r) => r.prepareDestroyPermanently()));
    });

    await Promise.all(
      ['is_database_init', ...NETWORK_ARR.map((net) => `is_${net.chainId}_${net.networkType}_token_init`)].map((i) => database.localStorage.remove(i))
    );
