...
describe('HardwareWalletRegistry', () => {
  it('registers and retrieves adapters by normalized type', () => {
    ...
    registry.register('BSIM', 'iccid-1', adapter);

    expect(registry.size).toBe(1);
    expect(registry.get('bsim')).toBe(adapter);
    expect(registry.has('BSIM', 'iccid-1')).toBe(true);
  });

  it('throws on duplicate registrations for same type', () => {
    const registry = new HardwareWalletRegistry();
    ...
    expect(() => registry.register('BSIM', undefined, createAdapter({ type: 'bsim' }))).toThrow(
      '[HardwareWalletRegistry] Adapter already registered for type BSIM',
    );
  });

  it('guards mismatched hardwareId lookups', () => {
    const registry = new HardwareWalletRegistry();
    registry.register('bsim', 'device-a', createAdapter({ type: 'bsim' }));

    expect(registry.get('bsim', 'device-b')).toBeUndefined();
    expect(registry.has('bsim', 'device-b')).toBe(false);
  });

  it('lists shallow copies with live capability lookups', () => {
    const registry = new HardwareWalletRegistry();
    const adapter = createAdapter({ type: 'bsim' });
    registry.register('bsim', 'device-a', adapter);

    const first = registry.list();
    expect(first).toEqual([{ type: 'bsim', hardwareId: 'device-a', capabilities: { type: 'bsim' } }]);
