import { encode, decode, validateBase32Address } from '@fluent-wallet/base32-address';
...
const formatCfxAddress = ({ address, networkId, addressType = 'user' }) => {
  ...
  return encode(address.toLowerCase().replace(/0x./, ADDR_TYPE_TO_PREFIX[addressType]), networkId);
};
...
class Balance {
  ...
  async getNativeBalance(hex) {
    const ret = await this.send(this.getBalanceMethod, [this.isCfx ? formatCfxAddress({ address: hex, networkId: this.networkId }) : hex, this.params]);
    console.log('ret', ret);
    return ret;
  }
  async getTokenBalance({ userAddress, tokenAddress }) {
    ...
    const ret = await this.send(this.callMethod, [{ from: userAddress, to: tokenAddress, data }, this.params]).then((r) =>
      iface777.decodeFunctionResult(this.callMethod, r)
    );
    console.log('ret', ret);
    return ret;
  }
  ...
  async getTokenBalances({ userAddress, tokenAddress, checkerAddress }) {
    ...
    const res = await this.send(this.callMethod, [{ from: userAddress, to: checkerAddress, data }, this.params]).then((r) =>
      iface777.decodeFunctionResult(this.callMethod, r)
    );
