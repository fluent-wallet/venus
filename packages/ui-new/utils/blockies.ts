...
function write(buffer: any, offs: any, ...data: any[]) {
  for (const arg of data) {
    for (let i = 0; i < arg.length; i++) {
      buffer[offs++] = arg.charAt(i);
    }
  }
}

function byte2(w: any) {
  return String.fromCharCode((w >> 8) & 255, w & 255);
}

function byte4(w: any) {
  return String.fromCharCode((w >> 24) & 255, (w >> 16) & 255, (w >> 8) & 255, w & 255);
}

function byte2lsb(w: any) {
...
function hsl2rgb(h: any, s: any, l: any): [number, number, number, number] {
  let r, g, b;
...
interface BlockiesOptions {
  ...
  color?: [any, any, any];
  bgcolor?: [any, any, any];
  spotcolor?: [any, any, any];
}

function buildOpts(opts: BlockiesOptions): BlockiesOptions {
  if (!opts.seed) {
...
class BlockiesPNG {
  ...
  constructor(width: any, height: any, depth: any) {
    this.width = width;
    this.height = height;
    this.depth = depth;
  ...
  public color(red: any, green: any, blue: any, alpha?: any) {
...
function _toDataUrl(_address: string | null | undefined) {
  ...
  if (opts.bgcolor?.length !== 3 || opts.color?.length !== 3 || opts.spotcolor?.length !== 3) {
    throw new Error('invalid opts');
  }

  const p = new BlockiesPNG(opts.size * opts.scale, opts.size * opts.scale, 3);
