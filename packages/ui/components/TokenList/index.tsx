...
import { ERC20tokenListAtom, FetchTokenListType, TokenListItem, requestTokenList } from './service';
...
const TokenList: React.FC<{ address: Address; onPress?: (v: TokenListItem) => void }> = ({ address, onPress }) => {
  const { theme } = useTheme();
  const [tokenList, setTokenList] = useAtom(ERC20tokenListAtom);
  useEffect(() => {
    firstValueFrom(requestTokenList(address.hex, FetchTokenListType.ERC20)).then((list) => {
      setTokenList(list);
    });
        ...
        <Pressable onPress={onPress ? () => onPress(item) : undefined}>
          <View className={'flex flex-row w-full justify-between p-3'}>
            <View className="flex flex-row items-center">
              {item.iconUrl ? <Image source={{ uri: item.iconUrl }} width={48} height={48} /> : <TokenIconUSDT width={48} height={48} />}
