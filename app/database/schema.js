...
export const DB_TABLE_SCHEMAS_OBJ = {
  hd_path: [
...
const DB_TABLE_SCHEMAS = Object.entries(DB_TABLE_SCHEMAS_OBJ).map(
  ([name, columns]) => {
    return {name, columns};
  },
);
const schema = appSchema({
  version: 1,
  tables: DB_TABLE_SCHEMAS.map(item => tableSchema(item)),
});
